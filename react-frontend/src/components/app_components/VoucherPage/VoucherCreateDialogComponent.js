...
const VoucherCreateDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [category_id], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.category_id)) {
                error["category_id"] = `Category field is required`;
                ret = false;
            }
            ...
            if (_.isEmpty(_entity?.points)) {
                error["points"] = `Points Required field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            category_id: _entity?.category_id?._id,title: _entity?.title,description: _entity?.description,points: _entity?.points,image: _entity?.image,
    ...
    return (
        <Dialog header="Create Voucher" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                ...
                <Dropdown id="category_id" value={_entity?.category_id?._id} optionLabel="name" optionValue="value" options={category_idOptions} onChange={(e) => setValByKey("category_id", {_id : e.value})}  required  />
            </span>
