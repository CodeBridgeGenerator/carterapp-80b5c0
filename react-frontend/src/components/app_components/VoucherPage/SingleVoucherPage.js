  ...
  const [isHelpSidebarVisible, setHelpSidebarVisible] = useState(false);
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((res) => {
                set_entity(res || {});
                const category_id = Array.isArray(res.category_id)
            ? res.category_id.map((elem) => ({ _id: elem._id, name: elem.name }))
            : res.category_id
                ? [{ _id: res.category_id._id, name: res.category_id.name }]
                : [];
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Points Required</label><p className="m-0 ml-3" >{_entity?.points}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Image URL</label><p className="m-0 ml-3" >{_entity?.image}</p></div>
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Category</label>
                    {category_id.map((elem) => (
    ...
    <div className="col-12 mt-2">
        <TabView>
                    ...
                    <TabPanel header="Cart Item History" leftIcon="pi pi-building-columns mr-2">
                        <CartItemHistoryPage/>
                    </TabPanel>
